...
<html lang="en">
  ...
  //DELETE
  if ( isset($_POST['delete'])&& isset($_POST['recordID']) ) {
    //checking if id exist
    $sql1="SELECT * FROM incomingordertable WHERE recordID = :a";
    $stmt1 = $pdo->prepare($sql1);
    $stmt1->execute(array(':a'=>$_POST['recordID']));
    $row1 = $stmt1->fetch(PDO::FETCH_ASSOC);
    if ($row1 == false ){
      $failure = "<span style='color:green'>User does not exist</span>";
    }
    else {
      $sql="DELETE FROM incomingordertable WHERE recordID = :a";
      $stmt = $pdo->prepare($sql);
      $stmt->execute(array(':a'=>$_POST['recordID']));
      $failure = "<span style='color:green'>Record deleted</span>";
    }
  }
?>
<head>
...
<body>
     ...
     <h2>DELETE RECORD</h2>
     <form method="post"><p>recordID to Delete:
       <input type="text" id="recordID" name="recordID" placeholder="Enter recordID here..." ></input>
